...
RSpec.describe 'Devise::Mailer' do
  describe 'notify' do
    ...
    it 'uses the user\'s name' do
      expect(mail.body.to_s).to include("Ciao #{CGI.escapeHTML(confirmable_user.name)},")
      expect(mail_body).to include("Ciao #{confirmable_user.name},")
    end
    ...
    context 'when the user name contains HTML' do
      ...
      it 'escapes the name in the rendered email body' do
        expect(mail.body.to_s).to include("Ciao #{CGI.escapeHTML(confirmable_user.name)},")
        expect(mail.body.to_s).not_to include("Ciao #{confirmable_user.name},")
      end
    end

    it 'shows the default confirmation state' do
      expect(mail_body).to include('Conferma la tua email per iniziare')
      expect(mail_body).to include('Benvenuto su Chatwoot. Dobbiamo solo verificare il tuo indirizzo email prima che tu possa iniziare a usare il tuo account.')
      expect(mail_body).to include('Conferma il mio account')
      expect(mail_body).not_to include('Invito allo spazio di lavoro')
    end
    ...
    context 'when there is an inviter' do
      ...
      it 'refers to the inviter and their account' do
        ...
        expect(mail_body).to include("#{inviter_val.name} ti ha invitato a unirti allo spazio di lavoro #{account.name} su Chatwoot.")
        expect(mail_body).to include('Accetta invito')
        expect(mail_body).not_to include('Conferma la tua email per iniziare')
      end
    ...
    context 'when user updates the email' do
      ...
      it 'sends a confirmation link' do
        ...
        expect(confirmation_body).to include('Conferma il tuo nuovo indirizzo email')
        expect(confirmation_body).to include('Nuova email')
    ...
    context 'when user already confirmed' do
      ...
      it 'send instructions with the link to login' do
        ...
        expect(confirmation_body).to include('Il tuo account e pronto')
        expect(confirmation_body).to include('Apri il mio account')
